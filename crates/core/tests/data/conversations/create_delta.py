...
# delta-rs does not currently materialize these configured checkpoint fields.
...
stats = [
    json.loads(add["stats"]) if add and add.get("stats") else None
    for add in adds
]
for parsed_stats in stats:
    if parsed_stats is not None:
        parsed_stats["tightBounds"] = True

inferred_stats_type = pa.array(stats).type
stats_field_order = [
    "numRecords",
    "minValues",
    "maxValues",
...
stats_type = pa.struct(
    [
        inferred_stats_type.field(name)
        for name in stats_field_order
        if inferred_stats_type.get_field_index(name) != -1
    ]
)
partition_values_type = pa.struct(
    [
        pa.field("_ACP_DATE", pa.date32()),
        pa.field("_ACP_BATCHID", pa.string()),
    ]
...
for add, parsed_stats in zip(adds, stats):
    if add is None:
        continue
    partition_values = dict(add["partitionValues"])
    add["stats_parsed"] = parsed_stats
    add["partitionValues_parsed"] = {
        "_ACP_DATE": date.fromisoformat(partition_values["_ACP_DATE"]),
        "_ACP_BATCHID": partition_values["_ACP_BATCHID"],
    }
...
patched_add_type = pa.struct(
    ...
    + [
        pa.field("stats_parsed", stats_type),
        pa.field("partitionValues_parsed", partition_values_type),
    ]
)
...
print(f"Created {TABLE_PATH} with {commit} versions and a patched checkpoint")
