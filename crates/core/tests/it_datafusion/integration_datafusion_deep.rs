use deltalake_core::delta_datafusion::DeltaNextPhysicalCodec;
use std::ops::Deref;
use std::sync::Arc;
use std::sync::Once;
...
use prost::Message;
use tracing::info;
use deltalake_core::delta_datafusion::{DeltaPhysicalCodec, DeltaScanExec};
        ...
        if let Some(exec) = plan.downcast_ref::<DataSourceExec>() {
            if let Some((file_scan, other)) = exec.downcast_to_file_source::<ParquetSource>() {
                if let Some(expr_adapter) = file_scan.expr_adapter_factory.clone() {
                    ...
                    assert!(debug_format.contains("DeltaDeepPhysicalExprAdapter"), "FileScanConfig does not have DeltaPhysicalExprAdapter after serde !");
                } else {
...
fn find_exec_node<T: ExecutionPlan + 'static>(input: &Arc<dyn ExecutionPlan>) -> Option<&T> {
    if let Some(found) = input.downcast_ref::<T>() {
        Some(found)
    } else {
        input.children().iter()
