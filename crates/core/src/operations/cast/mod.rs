...
pub fn cast_record_batch(
    ...
    let struct_array = if batch.num_rows() > 0 && batch.columns().len() == 0 && batch.schema().fields.len() == 0 {
        make_empty_struct_with_row_count(target_schema.fields(), &cast_options, add_missing, batch.num_rows())?
    } else {
