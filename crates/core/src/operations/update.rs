        ...
        .map(|field| {
            let expr = match updates.get(field.name()) {
                ...
                Some(expr) => {
                    ...
                    //     column) fail in strict mode or become null in safe mode.
                    let typed_expr = if safe_cast {
                        try_cast(expr.to_owned(), field.data_type().clone())
                    } else {
                        cast(expr.to_owned(), field.data_type().clone())
                    };
