    ...
    fn call(&self, args: &[Expr]) -> Result<Arc<dyn TableProvider>> {
        ...
        let mut args_string = args
            ...
            .map(|arg| match arg.clone() {
                Expr::Literal(ScalarValue::Utf8(Some(path)))
                | Expr::Literal(ScalarValue::LargeUtf8(Some(path)))
                | Expr::Literal(ScalarValue::Utf8View(Some(path))) => Ok(path),
        ...
        let table = std::thread::spawn(move || {
            ...
            rt.block_on(async {
                open_table_with_storage_options(&path, settings)
                    .await
                    .map_err(|e| {
