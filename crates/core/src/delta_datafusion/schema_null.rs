...
fn init() {
    if let Ok(var) = std::env::var("DELTA_FIELD_PATHS_TO_MAKE_NULLABLE") {
        let splits = var.split(",").map(|s| s.to_string()).collect::<Vec<_>>();
...
fn rewrite_field_type(field: &Field, paths: &[String], current_path: &str) -> Field {
    ...
    Field::new(field.name(), new_dt, field.is_nullable()).with_metadata(field.metadata().clone())
}

fn rewrite_data_type(dt: &DataType, paths: &[String], current_path: &str) -> DataType {
    match dt {
        Struct(fields) => Struct(Fields::from(rewrite_fields(fields, paths, current_path))),
        List(inner) => List(Arc::new(rewrite_field_type(inner, paths, current_path))),
        LargeList(inner) => LargeList(Arc::new(rewrite_field_type(inner, paths, current_path))),
        FixedSizeList(inner, n) => {
            FixedSizeList(Arc::new(rewrite_field_type(inner, paths, current_path)), *n)
        }
        ListView(inner) => ListView(Arc::new(rewrite_field_type(inner, paths, current_path))),
        LargeListView(inner) => {
            LargeListView(Arc::new(rewrite_field_type(inner, paths, current_path)))
        }
        Map(inner, sorted) => Map(
            Arc::new(rewrite_field_type(inner, paths, current_path)),
            *sorted,
        ),
        Dictionary(key, value) => Dictionary(
            key.clone(),
            Box::new(rewrite_data_type(value, paths, current_path)),
        ),
        ...
        Union(union_fields, mode) => {
            ...
            let fields: Vec<FieldRef> = union_fields
                .iter()
                .map(|(_, field)| {
                    ...
                    let make_nullable =
                        !field.is_nullable() && paths.iter().any(|p| *p == field_path);
...
mod tests {
    ...
    fn test_schema_null_empty_paths_returns_input() {
        let schema = make_schema(vec![Field::new("a", DataType::Int32, false)]);
    ...
    fn test_schema_null_already_nullable_unchanged() {
        let schema = make_schema(vec![Field::new("a", DataType::Int32, true)]);
        let result = rewrite_schema_with_nullable_fields_inner(schema, &paths(&["a"]));
        assert!(result.field_with_name("a").unwrap().is_nullable());
    }
    ...
    fn test_schema_null_deeply_nested_struct() {
        let schema = make_schema(vec![Field::new(
            ...
            Struct(Fields::from(vec![Field::new(
                ...
                Struct(Fields::from(vec![Field::new(
                    "c",
                    Struct(Fields::from(vec![Field::new("d", DataType::Int32, false)])),
                    false,
                )])),
                false,
            )])),
        ...
        let b = match a.data_type() {
            Struct(f) => f.find("b").unwrap().1,
            _ => panic!(),
        };
        assert!(!b.is_nullable());
        let c = match b.data_type() {
            Struct(f) => f.find("c").unwrap().1,
            _ => panic!(),
        };
        assert!(!c.is_nullable());
        let d = match c.data_type() {
            Struct(f) => f.find("d").unwrap().1,
            _ => panic!(),
        };
    ...
    fn test_schema_null_struct_itself_made_nullable() {
        let schema = make_schema(vec![Field::new(
            "a",
            Struct(Fields::from(vec![Field::new("b", DataType::Int32, false)])),
            false,
        )]);
        let result = rewrite_schema_with_nullable_fields_inner(schema, &paths(&["a"]));
        let a = result.field_with_name("a").unwrap();
    ...
    fn test_schema_null_list_itself_made_nullable() {
        let schema = make_schema(vec![Field::new(
            "a",
            List(Arc::new(Field::new("item", DataType::Int32, false))),
            false,
        )]);
        let result = rewrite_schema_with_nullable_fields_inner(schema, &paths(&["a"]));
        assert!(result.field_with_name("a").unwrap().is_nullable());
    ...
    fn test_schema_null_dictionary_with_nested_value() {
        let schema = make_schema(vec![Field::new(
            ...
            Dictionary(
                ...
                Box::new(Struct(Fields::from(vec![Field::new(
                    "b",
                    DataType::Utf8,
                    false,
                )]))),
