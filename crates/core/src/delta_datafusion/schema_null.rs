...
#[ctor::ctor]
fn init() {
    let paths_flag = if let Ok(v) = std::env::var("DELTA_FIELD_PATHS_TO_MAKE_NULLABLE") {
        v
    } else {
        "_acp_system_metadata.rowId,_acp_system_metadata.rowVersion,_acp_system_metadata.ingestTime,_acp_system_metadata.trackingId".to_string()
    };

    let splits = paths_flag
        .split(",")
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
