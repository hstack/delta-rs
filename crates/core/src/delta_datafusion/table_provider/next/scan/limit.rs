...
impl LimitPruneState {
    ...
    pub(crate) fn finalize(mut self) -> (Vec<ScanFileContext>, usize) {
        if self.rows_collected < self.limit {
            self.files.extend(self.unknown.drain(..));
            (self.files, 0)
        } else {
            let pruned = self.unknown.len();
            (self.files, pruned)
        }
    }
}
...
mod tests {
    ...
    fn stub_file(url: &str, num_rows: Precision<usize>, has_dv: bool) -> ScanFileContext {
