...
pub use self::scan::{DeltaNextPhysicalCodec, DeltaScanExec};
...
use crate::logstore::{StorageConfig, object_store_factories};
...
impl TableProvider for DeltaScan {
    ...
    async fn scan_with_args<'a>(
        ...
        if state
            .runtime_env()
            .object_store(table_uri.as_object_store_url())
            .is_err()
        {
            ...
            if let Some(entry) = object_store_factories().get(&url_key) {
                let storage_config =
                    StorageConfig::parse_options(&self.snapshot.snapshot().load_config().options)?;
        ...
        scan::execution_plan(&self.config, state, scan_plan, stream, engine, args.limit())
            .await
            .map(ScanResult::new)
    }
